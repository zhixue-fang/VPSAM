...
class ImageEncoderViT(nn.Module):
    def __init__(
        ...
        self.decam_embed = nn.Identity()
        self.spatial_Adapter = qkvAttention(dim=embed_dim, num_heads=num_heads)
        self.temporal_cnn_Adapter = SingleConv(in_channels=embed_dim, out_channels=embed_dim)
        self.temporal_atth_Adapter = qkvAttention(dim=embed_dim, num_heads=num_heads)
        self.cam_cnn_Adapter = SingleCNNEmbed(patchsize=patch_size, in_chans=3, embed_dim=embed_dim)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        camx = self.fourier_transformer(x)
        camx = self.cam_cnn_Adapter(camx)  # b h w c
        ...
        d_x = torch.pow(x - camx, 2)
        x = torch.einsum("bhwc, bhwc -> bhwc", x, d_x)
        ...
        cnnx = cnnx * x
        aligned_cnnx = self.spatial_Adapter(cnnx, cnnx, cnnx)
        bias = self.temporal_cnn_Adapter(aligned_cnnx.permute(0, 3, 1, 2)).permute(0, 2, 3, 1)
        spatio_temproal_x = self.temporal_atth_Adapter(aligned_cnnx, bias, bias)

        for blk in self.blocks:
            x, spatio_temproal_x = blk(x, spatio_temproal_x) # b h w c

        x = x + 0.5 * spatio_temproal_x
...
class FourierTransformer(nn.Module):
    ...
    def forward(self, x):
        b, _, h, w = x.shape
        fre = torch.fft.fft2(x)
        fre_m = torch.abs(fre)
        fre_p = torch.angle(fre)

        p_const = fre_p.mean()
        fre_ = fre_m * torch.exp(1j * p_const)
        re_img = torch.abs(torch.fft.ifft2(fre_, dim=(-2, -1)))
        return re_img
...
class ParaBlock(nn.Module):
    ...
    def __init__(
        ...
        if self.window_size == 0:
            ...
            self.SpatioTemporal_Adapter = qkvAttention(dim=dim, num_heads=num_heads)  # with skip connection
    ...
    def forward(self, x: torch.Tensor, cnnx: torch.Tensor,) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        if self.window_size == 0:
            stx = self.SpatioTemporal_Adapter(x, cnnx, cnnx)  # b h w c
            x = x + stx
